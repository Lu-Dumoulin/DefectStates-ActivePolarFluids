% Two-panel figure for the polar (radial) model -- LOG-LOG inset variant.
% Identical to fig_active_polar.tex except the inset (max_r v vs tau) is a log-log plot.
...
\def\rhobar{0.8148}\def\arho{0.2768}% \arho = 1/lambda_rho^2
\def\ap{1.2132}%                       \ap  = 1/lambda_p
\def\av{0.5420}%                       \av  = 1/tau_v
\begin{tikzpicture}[font=\footnotesize]
  ...
  % --- inset in the fields panel: maximal velocity vs turnover time (LOG-LOG) ---
  ...
  \begin{loglogaxis}[
      ...
      log ticks with fixed point,        % show 0.1, 1, ... instead of 10^{-1}
      xtick={0.1,1}, ytick={0.03,0.1,0.3},
